...
import { Calendar, Copy, Flame, SquareCheckBig, Trash } from "lucide-react"
...
export default function TaskCard({ item, onDelete }: Props) {
    ...
    const handleShare = (id: number) => {
        const url = `${window.location.origin}/project/${params?.id}?project=${params?.id}&task=${id}`
    ...
    return (
        ...
        >
            <CardHeader className="p-3 pb-0 flex space-y-0 flex-row justify-between items-start gap-3">
                <AvatarGroup
                    max={4}
                    total={item?.users_data?.length}
                    countClass={cn("h-7 w-7")}
                >
                ...
                <div
                    className={cn(
                        ...
                        getPriorityColor(item.priority),
                        item.priority === 3 && "w-8 h-8",
                    )}
                >
                    <Flame
                        className={cn(
                            "w-4 h-4",
                            item.priority === 3 && "w-6 h-6",
                            item.priority === 2 && "w-5 h-5",
                        )}
                    />
                </div>
            </CardHeader>
